...
def _classify(per_pm: list[dict]) -> dict:
    ...
    Returns dict with:
      ...
      pm_count, bullish, bearish, neutral
      confidence_floor: min PM confidence (None if no signals)
      low_conf_pms: list of agent keys with confidence < CONFIDENCE_FLOOR
      members: full per-pm list (for downstream reporting)
    """
    if len(per_pm) < 2:
        return {
            "type": "insufficient",
    ...
    bull = sum(1 for p in per_pm if p["signal"] == "bullish")
    bear = sum(1 for p in per_pm if p["signal"] == "bearish")
    neu = sum(1 for p in per_pm if p["signal"] == "neutral")

    confs = [p["confidence"] for p in per_pm if isinstance(p["confidence"], (int, float))]
    conf_floor = min(confs) if confs else None
    low_conf = [
        p["agent"] for p in per_pm
        if isinstance(p["confidence"], (int, float)) and p["confidence"] < CONFIDENCE_FLOOR
    ]
